...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!student_id || !total_goal || total_goal <= 0) {
      return noCacheJson({ error: '올바른 목표를 입력해주세요.' }, { status: 400 });
    }
    ...
    if (!student) {
      return noCacheJson({ error: '학생을 찾을 수 없습니다.' }, { status: 404 });
    }
    ...
    // 수정 횟수 체크
    if (editCount >= 2) {
      return noCacheJson({ error: '목표 수정은 최대 2회까지만 가능합니다. 선생님께 문의하세요.' }, { status: 403 });
    }

    // 1회 이상 수정한 경우 상향만 가능
    if (editCount >= 1 && total_goal <= (student.total_goal ?? 0)) {
      return noCacheJson({ error: '기존 목표보다 높은 값으로만 변경할 수 있습니다.' }, { status: 400 });
    }
