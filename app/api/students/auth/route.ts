...
export async function POST(request: NextRequest) {
  try {
    ...
    if (action === 'search') {
      ...
      return noCacheJson(results);
    }

    // PIN 설정 (최초)
    if (action === 'set_pin') {
      if (!student_id || !pin || pin.length !== 4) {
        return noCacheJson({ error: 'PIN은 4자리 숫자입니다.' }, { status: 400 });
      }
      const { data: student } = await supabase
        .from('students').select('pin_code').eq('id', student_id).single();
      if (student?.pin_code) {
        return noCacheJson({ error: '이미 PIN이 설정되어 있습니다.' }, { status: 400 });
      }
    ...
    if (action === 'login') {
      if (!student_id || !pin) {
        return noCacheJson({ error: '이름과 PIN을 입력해주세요.' }, { status: 400 });
      }
      const { data: student } = await supabase
        .from('students')
        .select('id, name, school, grade, pin_code, total_goal, goal_edit_count')
        .eq('id', student_id)
        .single();
      if (!student) return noCacheJson({ error: '학생을 찾을 수 없습니다.' }, { status: 404 });
      if (student.pin_code !== pin) return noCacheJson({ error: 'PIN이 일치하지 않습니다.' }, { status: 401 });
      return noCacheJson({
        success: true,
    ...
    return noCacheJson({ error: 'invalid action' }, { status: 400 });
  } catch (e: unknown) {
